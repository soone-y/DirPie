...
struct DeltaInfo {
  ...
  uint64_t showUntilTick = 0;
};

static std::unordered_map<wstring, uint64_t> g_lastShownBytes;
static std::unordered_map<wstring, DeltaInfo> g_deltaByPath;
static std::unordered_map<wstring, uint64_t> g_baselineAfterScanBytes;
static std::unordered_map<wstring, bool> g_changedAfterScan;

static uint64_t g_scanStartTick = 0;
static uint64_t g_scanDoneTick = 0;
static bool g_scanFullyComplete = false;
static bool g_haveScanStart = false;
static SYSTEMTIME g_scanStartLocal{};
static SYSTEMTIME g_lastUpdateLocal{};


static uint64_t NowTick() { return GetTickCount64(); }

static wstring GetExeDir() {
  wchar_t exePath[MAX_PATH]{};
  GetModuleFileNameW(nullptr, exePath, MAX_PATH);
  wstring p = exePath;
  const size_t pos = p.find_last_of(L"\\/");
  if (pos != wstring::npos) p = p.substr(0, pos);
  return p;
}

static wstring FormatLocalTimeHHMMSS(const SYSTEMTIME& st) {
  wchar_t buf[32];
...
static void ResetScanUxState() {
  g_lastShownBytes.clear();
  g_deltaByPath.clear();
  g_baselineAfterScanBytes.clear();
  g_changedAfterScan.clear();
  g_scanStartTick = NowTick();
  g_scanDoneTick = 0;
  g_scanFullyComplete = false;
  GetLocalTime(&g_scanStartLocal);
  g_lastUpdateLocal = g_scanStartLocal;
  g_haveScanStart = true;
}
...
static void UpdateWindowTitleProgress() {
  ...
  const uint32_t doneRaw = g_jobs_done.load();
  const uint32_t done = (total > 0 && doneRaw > total) ? total : doneRaw;
  const uint32_t active = g_jobs_active.load();
  const uint32_t queued = g_jobs_queued.load();

  bool scanning = (active + queued) > 0 && total > 0 && done < total;

  wchar_t title[256];
  if (scanning) {
    swprintf(title, 256, L"DirPie4  (Scanning %u/%u)", done, total);
  } else {
...
static void RefreshUIFromCache(uint64_t gen) {
  ...
  GetLocalTime(&g_lastUpdateLocal);
  ...
  for (int i = 0; i < (int)g_entries.size(); ++i) {
    ...
    const uint64_t nowTick = NowTick();

    if (!e.has_value) {
      sSize = L"...";
    } else {
      // delta (short-lived)
      auto itPrev = g_lastShownBytes.find(e.path);
      if (itPrev != g_lastShownBytes.end() && itPrev->second != e.bytes) {
        DeltaInfo di;
        di.deltaBytes = (long long)e.bytes - (long long)itPrev->second;
        di.showUntilTick = nowTick + 2500;
        g_deltaByPath[e.path] = di;

        // If scan had been fully complete, mark as changed-after-scan (content changed).
        if (g_scanFullyComplete) {
          auto itBase = g_baselineAfterScanBytes.find(e.path);
          if (itBase != g_baselineAfterScanBytes.end() && itBase->second != e.bytes) {
            g_changedAfterScan[e.path] = true;
          }
        }
      }
      g_lastShownBytes[e.path] = e.bytes;

      bool approx = (!e.exact) || e.incomplete;
      sSize = (approx ? L"~ " : L"") + FormatBytes(e.bytes);
      if (e.incomplete) sSize += L"  +";

      // delta annotation (only while active)
      auto itD = g_deltaByPath.find(e.path);
      if (itD != g_deltaByPath.end() && nowTick < itD->second.showUntilTick && itD->second.deltaBytes != 0) {
        const long long d = itD->second.deltaBytes;
        const uint64_t ad = (uint64_t)(d < 0 ? -d : d);
        sSize += L"  (Δ ";
        sSize += (d >= 0 ? L"+" : L"-");
        sSize += FormatBytes(ad);
        sSize += L")";
      }

      // changed-after-scan marker
      auto itCh = g_changedAfterScan.find(e.path);
      if (itCh != g_changedAfterScan.end() && itCh->second) {
        sSize += L"  ↻";
      }
  ...
  const uint32_t doneJobsRaw = g_jobs_done.load();
  const uint32_t activeJobs = g_jobs_active.load();
  const uint32_t queuedJobs = g_jobs_queued.load();

  const uint32_t doneJobs = (totalJobs > 0 && doneJobsRaw > totalJobs) ? totalJobs : doneJobsRaw;
  const uint32_t exactTotal = g_jobs_exact_total.load();
  const uint32_t exactDone = g_jobs_exact_done.load();

  // Determine whether this scan is fully complete (no missing, no cap, no permission skips).
  const bool fullyNow = (activeJobs + queuedJobs == 0) && (knownEntries == totalEntries) && (incompleteEntries == 0) && (!totals.incomplete);

  if (!g_scanFullyComplete && fullyNow) {
    g_scanFullyComplete = true;
    g_scanDoneTick = NowTick();
    g_baselineAfterScanBytes.clear();
    g_changedAfterScan.clear();
    for (const auto& e : g_entries) {
      if (e.has_value) g_baselineAfterScanBytes[e.path] = e.bytes;
    }
  }
  ...
  if (activeJobs + queuedJobs > 0 && totalJobs > 0) {
    swprintf(sbuf, 512,
             L"%s  |  scanning %u/%u (active=%u queued=%u exact=%u/%u)  |  known %d/%d  |  skipped access=%u path=%u other=%u reparse=%u%s  |  start %s  |  update %s",
             g_currentDir.c_str(),
             doneJobs, totalJobs, activeJobs, queuedJobs, exactDone, exactTotal,
             ...
             g_haveScanStart ? FormatLocalTimeHHMMSS(g_scanStartLocal).c_str() : L"--:--:--",
             FormatLocalTimeHHMMSS(g_lastUpdateLocal).c_str());
  } else {
    int changedCount = 0;
    for (const auto& kv : g_changedAfterScan) if (kv.second) ++changedCount;

    swprintf(sbuf, 512,
             L"%s  |  done%s  |  known %d/%d  |  skipped access=%u path=%u other=%u reparse=%u%s  |  start %s  |  update %s%s",
             g_currentDir.c_str(),
             g_scanFullyComplete ? L"" : L" (partial)",
             knownEntries, totalEntries,
             totals.skipped_access, totals.skipped_path, totals.skipped_other, totals.skipped_reparse,
             totals.incomplete ? L"  (incomplete)" : L"",
             g_haveScanStart ? FormatLocalTimeHHMMSS(g_scanStartLocal).c_str() : L"--:--:--",
             FormatLocalTimeHHMMSS(g_lastUpdateLocal).c_str(),
             (changedCount > 0) ? L"  |  changed" : L"");
  }
  SetStatusText(sbuf);
...
static std::vector<SliceGeom> ComputeSlices(float baseStartDeg,
  ...
  for (size_t i = 0; i < bytes.size(); ++i) {
    float sweep = 0.0f;
    if (sum > 0) sweep = (float)(totalSweepDeg * ((double)bytes[i] / (double)sum));
    out[i] = { angle, sweep };
    angle += sweep;
  }
  return out;
...
static bool AngleInSweep(double angDeg, float startDeg, float sweepDeg) {
  if (sweepDeg <= 0.0f) return false;
  ...
  float e = s + sweepDeg;
  if (e <= 360.0f) return ((float)angDeg >= s) && ((float)angDeg < e);
  return ((float)angDeg >= s) || ((float)angDeg < (e - 360.0f));
}

static void FillPieWrapped(Gdiplus::Graphics& g, Gdiplus::Brush* br,
                           const Gdiplus::Rect& rect, float startDeg, float sweepDeg) {
  if (sweepDeg <= 0.0f) return;
  float s = startDeg;
  while (s < 0.0f) s += 360.0f;
  while (s >= 360.0f) s -= 360.0f;

  if (s + sweepDeg <= 360.0f) {
    g.FillPie(br, rect, s, sweepDeg);
  } else {
    const float first = 360.0f - s;
    const float second = sweepDeg - first;
    g.FillPie(br, rect, s, first);
    if (second > 0.0f) g.FillPie(br, rect, 0.0f, second);
  }
}
...
static int HitTestPie(POINT ptClient) {
  ...
  // 3/4 ring while not fully complete
  const bool fullyComplete = g_scanFullyComplete;
  const float base = fullyComplete ? 0.0f : 135.0f;
  const float sweepTotal = fullyComplete ? 360.0f : 270.0f;

  if (!fullyComplete) {
    // Gap is [45,135) degrees.
    if (ang >= 45.0 && ang < 135.0) return -1;
  }

  const auto slices = ComputeSlices(base, sweepTotal, bs, sum);
  for (int i = 0; i < (int)slices.size(); ++i) {
    if (AngleInSweep(ang, slices[i].startDeg, slices[i].sweepDeg)) return i;
  }
  return -1;
}
...
static void PiePaint(HWND hwnd, HDC hdc) {
  ...
  // Background ring area (also shows 3/4 state when not complete)
  {
    Gdiplus::SolidBrush bg(Gdiplus::Color(255, 220, 220, 220));
    FillPieWrapped(g, &bg, pieRect, base, sweepTotal);
  }

  if (allKnown && sum > 0) {
    const auto slices = ComputeSlices(base, sweepTotal, bs, sum);
    for (int i = 0; i < (int)slices.size(); ++i) {
      if (slices[i].sweepDeg <= 0.0f) continue;
      Gdiplus::SolidBrush br(SliceColor(i));
      FillPieWrapped(g, &br, pieRect, slices[i].startDeg, slices[i].sweepDeg);
    }
  }
  ...
  if (activeJobs + queuedJobs > 0 && totalJobs > 0) {
    wchar_t buf[96];
    swprintf(buf, 96, L"Scanning %u/%u", doneJobs, totalJobs);
    center = buf;
...
int APIENTRY wWinMain(
  ...
  // Default initial folder: the folder containing this executable
  g_currentDir = GetExeDir();
  ...
  if (lpCmdLine && lpCmdLine[0]) {
    ...
    if (!p.empty()) g_currentDir = TrimTrailingSlash(p);
  }
